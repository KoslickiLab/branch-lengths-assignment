...
#  out_file="$output_dir"/results/"$base_name"_no_perturb_recover_nnls.txt
#  python solve_branch_lengths.py -m nnls -t $tree -A $A_file -y $y_file -l $edge_file -o $out_file -f 5
#done
...
#  basis_file="$output_dir"/"$base_name"_no_perturb_leaf_nodes.npy
#  out_file="$output_dir"/results/"$base_name"_no_perturb_recover_bottom-up.txt
...
do
  ...
  echo $base_name
  bu_result="$output_dir"/results/"$base_name"_no_perturb_recover_bottom-up.txt
  bu_out="$output_dir"/results/"$base_name"_compare_bottom-up.png
  python compare_edge_list.py -r $ref -i $bu_result -o $bu_out -c dodgerblue
  nnls_result="$output_dir"/results/"$base_name"_no_perturb_recover_nnls.txt
  nnls_out="$output_dir"/results/"$base_name"_compare_nnls.png
  python compare_edge_list.py -r $ref -i $nnls_result -o $nnls_out -c coral
done
